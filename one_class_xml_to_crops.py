...
if __name__ == '__main__':
    ...
    os.makedirs(os.path.join(args.output_path, 'hxq_ywyc'), exist_ok=True)
    ...
    for xml_filename in xml_filenames:
        if 'xml' in xml_filename:
            ...
            for child in root.iter(tag='object'):
                ...
                crop = image[ymin: ymax, xmin: xmax]
                cv2.imwrite(os.path.join(os.path.join(args.output_path, name.text, '{:0>6d}.jpg'.format(count))),
                            crop)
                # cv2.imshow('crop', crop)
                # cv2.waitKey(1)
                count += 1